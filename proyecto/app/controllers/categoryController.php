...
class categoryController extends mainModel{
    public function registrarCategoriaControlador(){
        ...
        $nombreCategoria = $this->limpiarCadena($_POST['nombre_categoria']);
        ...
        if($this->verificarDatos("[a-zA-ZáéíóúÁÉÍÓÚñÑ0-9 ]{3,40}",$nombreCategoria)){
            $alerta = [
                "tipo"=>"simple",
                "titulo"=>"Ocurrio un error inesperado",
        ...
        $categoria_datos_reg = [
            [
                ...
                "campo_marcador"=>":NombreCategoria",
        ...
        if($registrar_categoria->rowCount()==1){ // funcion para ver si el usuario se registro
            
            
            $alerta = [
                "tipo"=>"limpiar",
                "titulo"=>"Categoria de productos registrada",
                "texto"=>"La categoria se creo con exito",
                "icono"=>"success"
            ];
    ...
    public function listarCategoriaControlador($pagina,$registros,$url,$busqueda){ /* pagina sera el numero total de paginas,
            ...
            foreach($datos as $rows){ // para recorrer e ir agregando todo los datos del array de los usuarios
                $tabla.='
                    <tr class="has-text-centered" >
                        ...
                        <td>
                            <a href="'.APP_URL.'categoryUpdate/'.$rows['id_categoria'].'/" class="button is-success is-rounded is-small">Actualizar</a>
                        </td>
                        <td>
    ...
    public function actualizarCategoriaControlador(){
        ...
        if($this->actualizarDatos("categoria",$categoria_datos_up,$condicion)){
            ...
            $alerta=[
                ...
                "texto"=>"La categoria ha sido actualizada",
                "icono"=>"success"
            ];
        }else{
            $alerta=[
                "tipo"=>"simple",
                "titulo"=>"Ocurrió un error inesperado",
                "texto"=>"No hemos podido actualizar los datos de la categoria",
                "icono"=>"error"
            ];
        }
    ...
    public function eliminarCategoriaControlador(){
        $id=$this->limpiarCadena($_POST['id_categoria']); // por seguridad limpiar el texto id


          
		    $eliminarCategoria=$this->eliminarRegistro("categoria","id_categoria",$id); // ejecutar la consulta para eliminar el usuario

		    if($eliminarCategoria->rowCount()==1){ // verificar si el usuario fue eliminado
